...
17. Click "calculate volumes", then move on to next muscle. All muscle metrics are stored in a .csv file, so you can work on some muscles in one session, close Blender, then continue with the remaining muscles in a different session. We recommend you click "calculate volumes" after each muscle so that all the data is stored in your file as soon as each muscle is done. If you change a muscle (e.g. scale the muscle belly etc), click "calculate volumes" again and the volumes in your .csv file will update.
